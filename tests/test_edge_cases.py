"""Edge-case and gap-filling tests for pyureq.
...
Covers features not exercised by test_basic.py or test_compat.py:
  ...
  - pyureq.request() generic method
  - pyureq.options() HTTP method
  - Edge cases: empty/None params, None cookies
"""

import pytest
import pyureq
from pyureq.exceptions import (
    HTTPError,
    RequestException,
    Timeout,
    TooManyRedirects,
...
class TestVerify:
    def test_verify_false_stateless(self):
        ...
        r = pyureq.get(f"{BASE}/get", verify=False)
...
class TestRaiseForStatus:
    def test_raises_on_400(self):
        r = pyureq.get(f"{BASE}/status/400")
...
class TestTimeout:
    def test_timeout_raises(self):
        with pytest.raises(Timeout):
            pyureq.get(f"{BASE}/delay/5", timeout=0.05)
...
class TestBadURLs:
    def test_missing_scheme(self):
        with pytest.raises(RequestException):
            pyureq.get("127.0.0.1/get")

    def test_completely_invalid_url(self):
        with pytest.raises(RequestException):
            pyureq.get("not_a_url_at_all")

    def test_unknown_scheme(self):
        with pytest.raises(RequestException):
            pyureq.get("ftp://example.com/file")
...
class TestResponseCookies:
    ...
    def test_cookies_parsed_from_set_cookie(self):
        r = pyureq.get(f"{BASE}/cookies/set", allow_redirects=False)
        assert r.status_code == 200
...
class TestGenericRequest:
    def test_request_get(self):
        r = pyureq.request("GET", f"{BASE}/get")
        assert r.status_code == 200

    def test_request_post(self):
        r = pyureq.request("POST", f"{BASE}/post", json={"x": 1})
        assert r.status_code == 200
    ...
    def test_request_delete(self):
        r = pyureq.request("DELETE", f"{BASE}/delete")
        assert r.status_code == 200
...
class TestOptions:
    def test_options_method(self):
        r = pyureq.options(f"{BASE}/options")
        assert r.status_code == 200

    def test_options_via_request(self):
        r = pyureq.request("OPTIONS", f"{BASE}/options")
        assert r.status_code == 200
...
class TestEdgeCaseParams:
    def test_none_params(self):
        r = pyureq.get(f"{BASE}/get", params=None)
        assert r.status_code == 200

    def test_empty_dict_params(self):
        r = pyureq.get(f"{BASE}/get", params={})
        assert r.status_code == 200

    def test_none_cookies(self):
        r = pyureq.get(f"{BASE}/get", cookies=None)
        assert r.status_code == 200

    def test_empty_dict_cookies(self):
        r = pyureq.get(f"{BASE}/get", cookies={})
        assert r.status_code == 200

    def test_none_headers(self):
        r = pyureq.get(f"{BASE}/get", headers=None)
        assert r.status_code == 200

    def test_list_of_tuple_params(self):
        r = pyureq.get(f"{BASE}/get", params=[("a", "1"), ("a", "2")])
        assert r.status_code == 200
        data = r.json()
